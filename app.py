...
st.write("Scans NSE F&O segments on the Daily (1D) Timeframe.")
...
if not results_df.empty:
    ...
    with st.spinner(f"Updating trend for {active_ticker}..."): st_matrix_df = get_supertrend_row(active_ticker)
    ...
    st.dataframe(st.dataframe(st_matrix_df.style.map(style_cells, subset=['Weekly', 'Daily', 'Hourly', '15 Min']), use_container_width=True, hide_index=True) if not st_matrix_df.empty else st.info("No data"))
else:
